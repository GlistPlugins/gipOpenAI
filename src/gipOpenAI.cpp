...
std::string gipOpenAI::getChatCompletion(std::string prompt, int maxTokens,
		int modelType, int temperature) {
	Json messages = Json::array( { Json::object( { { "role", "user" }, {
			"content", prompt } }) });
	Json j = { { "model", modeltype[modelType] }, { "messages", messages }, {
...
std::string gipOpenAI::getTextCompletion(std::string prompt, int maxTokens,
		int modelType, int temperature) {
	Json j = { { "model", modeltype[modelType] }, { "prompt", prompt }, {
			"max_tokens", maxTokens }, { "temperature", temperature } };
...
std::string gipOpenAI::getTextEdit(std::string input, std::string instruction,
		int modelType) {
	Json j = { { "model", modeltype[modelType] }, { "input", input }, {
...
std::vector<std::string> gipOpenAI::getImage(std::string prompt, int num,
		std::string size) {
	Json j = { { "prompt", prompt }, { "n", num }, { "size", size }, {
			"response_format", "b64_json" } };
	auto image = openai::image().create(j); // Using initializer lists
	std::vector < std::string > data;
	for (int i = 0; i < num; i++) {
		gFile ifile;
		ifile.load(gImage::generateDownloadedImagePath(),
				gFile::FILEMODE_WRITEONLY, true);
...
std::vector<std::string> gipOpenAI::getImageVariation(std::string imageFullPath,
		int num, std::string size) {
	Json j = { { "image", imageFullPath }, { "n", num }, { "size", size }, {
			"response_format", "b64_json" } };
...
std::string gipOpenAI::getCode(std::string prompt, int maxTokens,
		int modelType) {
	Json j = { { "model", modeltype[modelType] }, { "max_tokens", maxTokens }, {
			"prompt", prompt } };
	auto completion = openai::completion().create(j);
//	std::cout << "Response is:\n" << completion.dump(2) << '\n';
	return completion["choices"][0]["text"];
...
std::string gipOpenAI::fineTuneModel(std::string file) {
	auto fineTune = openai::fineTune().create( { { "training_file", file }, { "model", "davinci" }, { "n_epochs", 15 }, { "batch_size", 3 }, { "learning_rate_multiplier", 0.3 }, });
...
std::string gipOpenAI::getCompletion(std::string prompt, int maxTokens, std::string modelType, int temperature) {
	Json j = { { "model", modelType }, { "prompt", prompt }, { "max_tokens", maxTokens }, { "temperature", temperature } };
	auto completion = openai::completion().create(j);
	std::cout << "Response is:\n" << completion.dump(2) << '\n';
	return completion["choices"][0]["text"];
}
